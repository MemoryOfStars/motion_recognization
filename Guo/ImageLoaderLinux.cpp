...
HRESULT ImageLoader::GetPNGFilesFromDir(const std::string& path, std::vector<std::string>& fileList)
{
	...
	if (dirStream) 
	{
		...
		while ((dirFile = readdir(dirStream)) != NULL) 
		{
			...
			{
				fileList.push_back(pngFilename);            //如果是png文件就入文件队列
			}
		}
		closedir(dirStream);
...
HRESULT ConvertYUVToDeckLinkVideoFrame(void* yuvData, IDeckLinkVideoFrame* deckLinkVideoFrame, int width, int height)
{
	HRESULT result  = E_FAIL;
	uint8_t*	deckLinkBuffer	= nullptr;                             //数据地址

	if (deckLinkVideoFrame->GetBytes((void**)&deckLinkBuffer) != S_OK)  //得到Frame数据结构的data buffer地址
	{
		fprintf(stderr, "Could not get DeckLinkVideoFrame buffer pointer\n");
		goto yuvexit;
	}
	memset(deckLinkBuffer, 0, width*height*2);
	memcpy(deckLinkBuffer, yuvData, width*height*2);
...
HRESULT ImageLoader::ConvertPNGToDeckLinkVideoFrame(const std::string& pngFilename, IDeckLinkVideoFrame* deckLinkVideoFrame)
{
	...
	fread(pngHeader, 1, 8, pngFile);   //读8个字节(png 文件头信息)
	if (png_sig_cmp(pngHeader, 0, 8))  //判断文件头信息是否合法
	{
	...
	rowBytes		= png_get_rowbytes(pngDataPtr, pngInfoPtr);  //每一行所占用的字节数
	...
	videoFrameOffsetX = videoFrameWidth > (uint32_t)width ? (videoFrameWidth - (uint32_t)width) / 2 : 0;//如果指定的输出图像的宽 > 图片的宽，offset=差的二分之一(居中)
	videoFrameOffsetY = videoFrameHeight > (uint32_t)height ? (videoFrameHeight - (uint32_t)height) / 2 : 0;  //如果输出模式的高 > 图片的高，指定offset使图片居中显示
	...
	for (uint32_t row = 0; row < height; ++row)
	{
		...
		if (row < imageOffsetY) //将row移动到指定的offset处
			continue;
		else if (row >= (imageOffsetY + videoFrameHeight))//已经写完了所有的图像数据到frame结构体的data buffer中
			break;
			
		memcpy(deckLinkBuffer + videoFrameOffsetX * 4, rowPtr + imageOffsetX * 4, rowBytesToCopy);//一次从rowPtr + offset处读取rowBytesToCopy个字节到deckLinkBuffer + offset处
