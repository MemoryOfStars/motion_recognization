...
def enqueue(rgbData):
	shmid = shmget(SHM_KEY_RGB, SHM_SIZE_RGB, 0o666)  
	if shmid < 0:  
...
def dequeue():
	shmid = shmget(SHM_KEY_YUV, SHM_SIZE_YUV, 0o666)  
	if shmid < 0:  
		print ("System not infected")
		return None
	else:   
...
def analyze():
	...
	while True:
		...
		yuv_img = np.array(tuple(map(ord, yuv_data)), dtype=np.uint8).reshape(1620,1920)
